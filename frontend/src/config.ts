...
export async function loadConfig(): Promise<Config> {
    ...
    try {
        ...
        if (!backendCanisterId && config.backend_canister_id === 'undefined') {
            console.warn('CANISTER_ID_BACKEND is not set, falling back to mock mode');
        }
        ...
        const fallbackConfig = {
            ...
            backend_canister_id: backendCanisterId || 'mock',
...
export async function createActorWithConfig(options?: CreateActorOptions): Promise<backendInterface> {
    ...
    if (isProduction) {
        if (!config.backend_canister_id || config.backend_canister_id === 'undefined' || config.backend_canister_id === 'mock') {
            throw new Error('Production Readiness Error: Backend canister ID is not configured. Failing safely.');
        }
    } else {
        if (!config.backend_canister_id || config.backend_canister_id === 'undefined' || config.backend_canister_id === 'mock') {
